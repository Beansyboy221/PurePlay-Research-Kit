...
class DatasetBuilder:
    ...
    def get_files_from_dir(self, directory: pydantic.DirectoryPath) -> list[pydantic.FilePath]:
        ...
        return [
            ...
            if pydantic.TypeAdapter(validated_paths.ParquetPath).validate_python(os.path.join(directory, file_path)) # Ew
        ]
    ...
    def check_consistency(self, datasets_list: list[dataset.FileDataset]) -> None:
        ...
        def check_fields(
                fields: list[str], 
                reference_params: pydantic.BaseModel, 
                check_datasets: list[dataset.FileDataset]
            ) -> None:
            for field in fields:
