...
class CNNAutoencoder(base.AutoencoderBase):
    def _define_layers(self):
        ...
        if self.model_params.hidden_layers == 1:
            ...
            for i in range(self.model_params.hidden_layers):
                ratio = i / (
                    self.model_params.hidden_layers - 1
                )  # Ratio from 0.0 (hidden_size) to 1.0 (latent_size)
                layer_size = max(
                    1,
                    int(
        ...
        for i in range(len(decoder_sizes) - 1):
            ...
            )  # Scale factor must match encoder stride
            decoder_layers.append(
                torch.nn.Conv1d(
