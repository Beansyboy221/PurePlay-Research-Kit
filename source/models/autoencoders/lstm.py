...
class LSTMAutoencoder(base.AutoencoderBase):
    def _define_layers(self):
        ...
        self.compressor = torch.nn.Linear(
            self.model_params.hidden_size * 2, self.model_params.latent_size
        ).compile()
        self.decompressor = torch.nn.Linear(
            self.model_params.latent_size, self.model_params.hidden_size
        ).compile()
        ...
        self.reconstructor = torch.nn.Linear(
            self.model_params.hidden_size, self.data_params.features_per_poll
        ).compile()
    ...
    def forward(self, input_window: torch.Tensor) -> torch.Tensor:
        ...
        final_states = context_vector.unsqueeze(0).repeat(
            self.model_params.hidden_layers, 1, 1
        )
        decoded_window, (hidden_state, cell_state) = self.decoder(
            repeat_vector, (final_states, final_states)
