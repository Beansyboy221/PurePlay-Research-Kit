...
class GRUBinaryClassifier(base.ClassifierBase):
    ...
    def forward(self, input_window: torch.Tensor) -> torch.Tensor:
        data: torch.Tensor
        data, hidden_state = self.feature_extractor(input_window)
        pooled_data = data.mean(1)  # Globally pool to get even coverage of the window
        logits: torch.Tensor = self.classifier(pooled_data)
