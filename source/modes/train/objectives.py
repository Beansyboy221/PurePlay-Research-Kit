...
from preprocessing import datamodule, scalers
from . import config, callbacks, helpers
    ...
    trainer = lightning.Trainer(
        ...
        callbacks=[
            ...
            lightning.pytorch.callbacks.ModelCheckpoint(
                monitor="val_loss", dirpath=trial_directory
            ),
            ...
            lightning.pytorch.callbacks.StochasticWeightAveraging(
                swa_lrs=model_params.learning_rate * config.swa_lr_factor
                or model_params.learning_rate
                * trial.suggest_float(
                    name="swa_lr_factor",
        ...
        profiler=lightning.pytorch.profilers.SimpleProfiler(
            dirpath=trial_directory, filename="performance_log"
        ),
        logger=lightning.pytorch.loggers.TensorBoardLogger(
            save_dir=config.save_dir, version=f"trial_{trial.number}"
        ),
    )
