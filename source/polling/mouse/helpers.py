...
mouse_button_listener = pynput.mouse.Listener( # Must be started for buttons to update.
...
mouse_move_listener = raw_mouse.RawMouseListener() # Must be started for moves to update.
...
def poll_mouse(mouse_whitelist: frozenset[binds.Bind]) -> list:
    ...
    if binds.Moves.DIRECTION_X in mouse_whitelist:
        poll.append(delta_x / magnitude if magnitude > 0 else 0.0)
    if binds.Moves.DIRECTION_Y in mouse_whitelist:
        poll.append(delta_y / magnitude if magnitude > 0 else 0.0)
    if binds.Moves.VELOCITY in mouse_whitelist:
