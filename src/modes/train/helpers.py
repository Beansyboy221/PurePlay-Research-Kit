...
from beanml import optimizers, config as model_config

from preprocessing import datamodule
from . import config, callbacks
...
def tune_batch_size(
    ...
    logger = logging.getLogger("Batch Size Tuning")
...
def suggest_model_params(
    ...
    optimizer = config.optimizer or trial.suggest_categorical(
        name="optimizer", choices=optimizers.SUPPORTED_OPTIMIZERS
    )
    scheduler = config.scheduler or trial.suggest_categorical(
        name="scheduler", choices=[None] + optimizers.SUPPORTED_SCHEDULERS
    )
    ...
    if hasattr(optimizer, "momentum"):
        momentum = config.momentum or trial.suggest_float(
            name="momentum", low=config.momentum_min, high=config.momentum_max
        )

    return model_config.ModelConfig(
        hidden_layers=config.hidden_layers
        or trial.suggest_int(
            name="hidden_layers",
        ...
        or trial.suggest_int(
            name="latent_size", low=config.latent_size_min, high=hidden_size
        ),
        dropout=config.dropout
        or trial.suggest_float(
            name="dropout", low=config.dropout_min, high=config.dropout_max
        ),
        optimizer=optimizer,
        scheduler=scheduler,
